...
import restaurantsFakerFactory from "./restaurantsFakerFactory";
...
const RestaurantsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = restaurantsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("restaurants").create(elem));
...
export default connect(null, mapDispatch)(RestaurantsFakerDialogComponent);
