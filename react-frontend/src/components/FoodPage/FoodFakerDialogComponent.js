...
import foodFakerFactory from "./foodFakerFactory";
...
const FoodFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = foodFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("food").create(elem));
...
export default connect(null, mapDispatch)(FoodFakerDialogComponent);
